...
export const list = async (req, res) => {
    ...
    try {
        ...
        const list = await BCA_3_23_26_Form.find(query)
        // console.log(bca3AdmissionList);
        res.render('BCA_3_23_26/admList', { list, status, noOfForms: list.length, user })
    } catch (error) {
...
export const listPost = async (req, res) => {
    try {
        ...
        if (findStuMobileNo !== '') {
            let foundStudent = await BCA_3_23_26_Form.find({ mobileNumber: findStuMobileNo })
            res.render('BCA_3_23_26/admList', { list: foundStudent, status: "Found", noOfForms: foundStudent.length, user })

        } else if (findStuMobileNo === '') {
            const foundStudent = await BCA_3_23_26_Form.find({ isPaid: true })
            res.render('BCA_3_23_26/admList', { list: foundStudent, status: "All", formAlert: "Please, Enter Mobile No", noOfForms: foundStudent.length, user })
        }
    } catch (error) {
...
export const studentDetail = async (req, res) => {
    try {
        ...
        const foundStudent = await BCA_3_23_26_Form.findOne({ _id: stuId })

        res.render('BCA_3_23_26/studentDetail', { foundStudent, user })
    } catch (error) {
...
export const studentEdit = async (req, res) => {
    try {
        ...
        const { stuId } = req.params
        const foundStudent = await BCA_3_23_26_Form.findOne({ _id: stuId })
        res.render("BCA_3_23_26/studentEdit", { user, foundStudent })
    } catch (error) {
...
export const admList = async (req, res) => {
    try {
        const studentsBySubject = await BCA_3_23_26_Form.find({ isPaid: true })
        ...
        res.setHeader("Content-Disposition", "attachment; filename=BCA_Part_3_23_26_Adm_List.csv");
