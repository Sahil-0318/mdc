...
const ugRegularSem2Potal202428 = async (req, res) => {
    try {
        let queries = req.query
        const user = await User.findOne({ _id: req.id })

        if (queries.isOn === "true") {
            await PortalOnOff.findOneAndUpdate({portal : "ugRegularSem2_24-28"}, { $set : {isOn : false} })
        }  

        if (queries.isOn === "false") {
            await PortalOnOff.findOneAndUpdate({portal : "ugRegularSem2_24-28"}, { $set : {isOn : true} })
        }

        const portal = await PortalOnOff.findOne({portal : "ugRegularSem2_24-28"})
        res.render('portalOnOff', { url : "ugRegularSem2Potal202428", session : "UG Regular Sem 2 (2024 - 28)", user, portal : portal.isOn })
    } catch (error) {
        console.log("Error in UG Regular Sem 2 Potal 2024-28 Get Method =====>", error)
    }
}
...
const bca2202427 = async (req, res) => {
    try {
        let queries = req.query
        const user = await User.findOne({ _id: req.id })

        if (queries.isOn === "true") {
            await PortalOnOff.findOneAndUpdate({portal : "bca2_24-27"}, { $set : {isOn : false} })
        }  

        if (queries.isOn === "false") {
            await PortalOnOff.findOneAndUpdate({portal : "bca2_24-27"}, { $set : {isOn : true} })
        }

        const portal = await PortalOnOff.findOne({portal : "bca2_24-27"})
        res.render('portalOnOff', { url : "bca2202427", session : "BCA Part 2 (2024 - 27)", user, portal : portal.isOn })
    } catch (error) {
        console.log("Error in BCA Part 2 Potal 2024-27 Get Method =====>", error)
    }
}
...
const ugRegSem5Potal202327 = async (req, res) => {
    try {
        let queries = req.query
        const user = await User.findOne({ _id: req.id })

        if (queries.isOn === "true") {
            await PortalOnOff.findOneAndUpdate({portal : "ugRegularSem5_23-27"}, { $set : {isOn : false} })
        }  

        if (queries.isOn === "false") {
            await PortalOnOff.findOneAndUpdate({portal : "ugRegularSem5_23-27"}, { $set : {isOn : true} })
        }

        const portal = await PortalOnOff.findOne({portal : "ugRegularSem5_23-27"})
        res.render('portalOnOff', { url : "ugRegSem5Potal202327", session : "UG Regular Sem 5 (2023 - 27)", user, portal : portal.isOn })
    } catch (error) {
        console.log("Error in UG Regular Sem 5 Potal 2023-27 Get Method =====>", error)
    }
}

const ugRegSem3Potal202428 = async (req, res) => {
    try {
        let queries = req.query
        const user = await User.findOne({ _id: req.id })

        if (queries.isOn === "true") {
            await PortalOnOff.findOneAndUpdate({portal : "ugRegularSem3_24-28"}, { $set : {isOn : false} })
        }  

        if (queries.isOn === "false") {
            await PortalOnOff.findOneAndUpdate({portal : "ugRegularSem3_24-28"}, { $set : {isOn : true} })
        }

        const portal = await PortalOnOff.findOne({portal : "ugRegularSem3_24-28"})
        res.render('portalOnOff', { url : "ugRegSem3Potal202428", session : "UG Regular Sem 3 (2024 - 28)", user, portal : portal.isOn })
    } catch (error) {
        console.log("Error in UG Regular Sem 3 Potal 2024-28 Get Method =====>", error)
    }
}
...
export {
    ...
    bca2202427,
    bca3202326,
    ugRegSem5Potal202327,
    ugRegSem3Potal202428
}